...
for (const file of cssFiles) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^\s*@(?:-webkit-)?keyframes\b/i.test(trimmed)) {
      skipKeyframes = true;
      braceDepth = (line.match(/{/g) || []).length - (line.match(/}/g) || []).length;
      if (braceDepth <= 0) skipKeyframes = false;
      continue;
    }

    const shouldRemove = propPatterns.some((pattern) => pattern.test(trimmed));
    if (shouldRemove) {
